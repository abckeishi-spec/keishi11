...
class GIJI_Admin_Manager {
    ...
    private function save_api_settings() {
        ...
        // APIキーの保存（暗号化）
        $api_keys = array('gemini_api_key', 'openai_api_key', 'claude_api_key');
        foreach ($api_keys as $key) {
            if (isset($_POST['giji_improved_' . $key]) && !empty($_POST['giji_improved_' . $key])) {
                $api_key = sanitize_text_field($_POST['giji_improved_' . $key]);
                $this->security_manager->save_api_key($key, $api_key);
            }
        }
