...
class Grant_Insight_JGrants_Importer_Improved {
    ...
    public function init_admin() {
        ...
        if (class_exists('GIJI_Admin_Manager')) {
            // 必要なコンポーネントを初期化（存在しない場合はダミーオブジェクト）
            $automation_controller = $this->automation_controller;
            $logger = $this->logger;
            $security_manager = $this->security_manager;
            ...
            if (!$security_manager) {
                $security_manager = new stdClass();
                $security_manager->check_admin_permission = function() {
                    if (!current_user_can('manage_options')) {
                        wp_die('この操作を実行する権限がありません。');
                    }
                };
                $security_manager->verify_nonce = function($nonce, $action) {
            ...
            if (!$automation_controller) {
                $automation_controller = new stdClass();
                $automation_controller->get_statistics = function() {
                    return array(
                        'draft_count' => 0,
                        'published_count' => 0,
                        'total_count' => 0,
            ...
            try {
                ...
                if ($logger && method_exists($logger, 'log')) {
                    $logger->log('管理画面を初期化しました');
                }
            } catch (Exception $e) {
                error_log('管理画面初期化エラー: ' . $e->getMessage());
                ...
                add_action('admin_menu', function() {
                    add_menu_page(
                        ...
                        function() {
                            echo '<div class="wrap">';
                            echo '<h1>Jグランツ・インポーター改善版</h1>';
                            echo '<div class="notice notice-warning"><p>プラグインの初期化中にエラーが発生しました。しばらくしてから再度お試しください。</p></div>';
                            echo '</div>';
                        },
                        'dashicons-money-alt',
                        30
            ...
            add_action('admin_menu', function() {
                add_menu_page(
                    ...
                    function() {
                        echo '<div class="wrap">';
                        echo '<h1>Jグランツ・インポーター改善版</h1>';
                        echo '<div class="notice notice-error"><p>管理画面クラスが見つかりません。プラグインファイルが正しくアップロードされているか確認してください。</p></div>';
                        echo '</div>';
                    },
                    'dashicons-money-alt',
                    30
